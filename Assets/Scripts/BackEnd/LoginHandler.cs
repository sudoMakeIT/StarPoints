...
public class LoginHandler : MonoBehaviour
{
    ...
    public InputField regPasswordText;
    ...
    {
        DebugLog(String.Format("Attempting to create user {0} {1} - {2}", emailReg, passwordReg, displayName));

        // This passes the current displayName through to HandleCreateUserAsync
        // so that it can be passed to UpdateUserProfile().  displayName will be
        // reset by AuthStateChanged() when the new user is created and signed in.
        string newDisplayName = displayName;
        auth.CreateUserWithEmailAndPasswordAsync(emailReg, passwordReg)
        .ContinueWith((task) =>
        {
    ...
    {
        DebugLog(String.Format("Attempting to sign in as {0} {1}...", email, password));
